...
traindf=pd.read_csv("./TrainFrameLabels.csv",dtype=str)
valdf=pd.read_csv("./ValidationFrameLabels.csv",dtype=str)

# creating an empty list
images = glob("./TrainFrames/*.jpg")
trainpart_image = []
train_image = []
trainpart_label = []
train_label = []
...
for i in tqdm(range(traindf.shape[0]-1400000)):
#	try:
	if i % 5 == 0:
		# loading the image and keeping the target size as (224,224,3)
		img = image.load_img("./TrainFrames/"+traindf['FrameID'][i], target_size=(100,100,3))
...
# creating an empty list
images = glob("./ValidationFrames/*.jpg")
...
# for loop to read and store frames
for i in tqdm(range(valdf.shape[0]-350000)):
#	try:
	if i % 5 == 0:
	    # loading the image and keeping the target size as (224,224,3)
	    img = image.load_img("./ValidationFrames/"+valdf['FrameID'][i], target_size=(100,100,3))
	    # converting it to array
	    img = image.img_to_array(img)
	    # normalizing the pixel value
	    img = img/255
...
model.add(TimeDistributed(BatchNormalization()))
model.add(TimeDistributed(Dropout(0.3)))
model.add(TimeDistributed(Conv2D(16, (5, 5),kernel_regularizer=l2(0.1),activation='relu')))
model.add(TimeDistributed(BatchNormalization()))
model.add(TimeDistributed(Dropout(0.3)))
model.add(TimeDistributed(Conv2D(16, (5, 5),kernel_regularizer=l2(0.1),activation='relu')))
model.add(TimeDistributed(BatchNormalization()))
...
model.add(LSTM(32, kernel_regularizer=l2(0.1), recurrent_regularizer=l2(0.1), return_sequences=True))
...
#path=r"/media/mel/MEL/Documents/Jobs/NP Industry Project - Coddie/Research on Behavioral Teaching/Face Image Datasets/DAiSEE"
testdf=pd.read_csv("./TestFrameLabels.csv",dtype=str)

# creating an empty list
images = glob("./TestFrames/*.jpg")
testpart_image = []
test_image = []
testpart_label = []
test_label = []
...
# for loop to read and store frames
for i in tqdm(range(testdf.shape[0]-300000)):
    if i % 5 == 0:
        # loading the image and keeping the target size as (224,224,3)
        img = image.load_img("./TestFrames/"+testdf['FrameID'][i], target_size=(100,100,3))
        # converting it to array
        img = image.img_to_array(img)
        # normalizing the pixel value
        img = img/255
...
send_notification_via_pushbullet("Time: "+str(datetime.now())+"\nModel_4.h5 Finished Evaluation", "Test Accuracy: "+str(test_acc)+"\nTest Loss: "+str(test_loss))
